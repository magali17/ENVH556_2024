...
1. `mesim_pure`: The simpler version to allow some basic understanding of pure Berkson and classical measurement error. It generates true exposure and health outcome data in a group of subjects, along with modifications of the true exposure to give mismeasured exposures, both pure Berkson and pure classical. You will find this on the class web page with the name: *mesim_pure.R*.  

2. `mesim_like`: The more complex version is based on the work done by Szpiro et al. It generates the exposure and health data, computes predictions, and estimates the quantities of interest that can be summarized, as described in Szpiro et al. You will find this on the class web page with the name: *mesim_like.R*.
...
The next chunk is to summarize the results using basic descriptive tools and plots.  You should build upon the tools you have learned earlier in the quarter
to conduct descriptive analyses and develop plots.  See some of the text in the practice session below for some more thoughts on what quantities you want to
...
Note:  For clarity of display we are presenting only 1,000 subjects.  However, the sample size of the subject dataset impacts the certainty of the parameter
estimates, so the slopes shown in this figure have more uncertainty than the slopes estimated in the simulation study with 10,000 subjects.  (Why?)
