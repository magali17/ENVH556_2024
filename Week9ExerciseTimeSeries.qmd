...
author: "Chris Zuidema, Elena Austin, and Instructors for ENVH 556 Autumn 2024"
format:
  html:
    df_print: "paged"
    fig_caption: yes
...
instrument_list <- list(gps = gps, 
                        ptrak_noscreen = ptrak_noscreen, 
                        ptrak_screen = ptrak_screen, 
                        co2 = co2)
...
A common task with time series data is averaging to different time scales. From the autocorrelation plot above, we can see at a lag of 60 seconds, the CO<sub>2</sub> concentrations are no longer correlated. **Let's convert our 1-second data to some longer time scales.** One package that helps with averaging tasks is `slider`.
...
ts_data <- ts_data %>% 
  mutate(
         ...
         # large differences indicate large conc changes over time
