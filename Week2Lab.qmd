...
# histogram + density w/ transparency + overlaid normal 
p +
...
Calculate the proportion of the samples that are greater than a threshold of log(200).


```{r param.exc.frac}
#-----param exc frac----

#  1. Convert log(200) into a z-score, to indicate how many standard deviations away from the mean it is. Standardize by subtracting the mean of lnrec and then dividing by its standard deviation.
# 2. pnorm() returns the probability that a normal random variable is less than or equal to the computed z-score
# 3. 1-pnmorm() gives the probability that this is greater than a z-score.

1 - pnorm((log(200) - mean(DEMSu$lnrec)) / sd(DEMSu$lnrec))

```

* **Upper 5^th^ percentile of the distribution and its 70% CI**.  The following describes an approach estimated directly on the log-transformed data and exponentiates.  You can use the `quantileCI` command in the `MKmisc` package. (See https://rdrr.io.)  The `quantile` command in base R can be used to get the upper 5th percentile (or 95th percentile), but there is no clear way to get its CI from the `quantile` command.
...
If the points in the lower tail (left side of the plot) lie *above* the reference line, it suggests that the actual data has more values that are smaller than expected under the log-normal distribution.

If the points in the upper tail (right side of the plot) lie *below* the reference line, it suggests that there are fewer extreme values in the dataset than would be expected under a log-normal distribution.
...
1) Plan to use an R project for this lab and make sure it is set up.  You probably will want to use one project for the entire ENVH556 course.
...
6) For a selected group (i.e. a single job, or for all four if you want): Assume data are lognormal (LN) and an OEL of 200 $\mu g/m^3$ has been determined for REC.
